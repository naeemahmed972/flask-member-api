...
from db_helpers import connect_db, get_db
...
def edit_member(member_id):
    ...
    member_cur = db.execute('SELECT id, name, email, level FROM members WHERE id = ?', [member_id])
    update_member = member_cur.fetchone()

    return jsonify({
        'member': {
            'id': update_member['id'],
            'name': update_member['name'],
            'email': update_member['email'],
...
def delete_member(member_id):
    ...
    return jsonify({'message': 'The member has been deleted successfully.'})
